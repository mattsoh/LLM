from torch import tensor, triu, ones, zeros, arange, softmax, argmax, sqrt, inf, tanh, pi, multinomial, topk, where, cat, no_grad

import torch.nn as nn
class GELU(nn.Module):
    def forward(self, x):
        return 0.5 * x * (1 + tanh(
            sqrt(tensor(2.0 / pi)) * (x + 0.044715 * x**3)
        ))
class GPT(nn.Module):
...
class MultiHeadAttn(nn.Module):
    def __init__(self, d_in, d_out, ctx_len, dropout, num_heads, qkv_bias=False):
        ...
        self.register_buffer('mask', triu(ones(ctx_len, ctx_len), diagonal=1))
...
class LayerNorm(nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
        self.eps = 1e-5
        self.scale = nn.Parameter(ones(emb_dim))
        self.shift = nn.Parameter(zeros(emb_dim))
...
def generate(model, idx, max_new_tokens, context_size, temperature=0.0, top_k=None, eos_id=None):
    for _ in range(max_new_tokens):
        ...
        if top_k is not None:
            ...
            logits = where(logits < min_val, tensor(float('-inf')).to(logits.device), logits)
